...
### `index.html` — 허브 + 전체 현황
- 상단 **계좌 바로가기** 카드 = 각 계좌 페이지로 가는 허브
- **전체 계좌 현황표**: `ACCOUNTS` 배열에 정의된 각 계좌의 데이터 파일을 fetch → 실시간 시세로 평가액(KRW) 환산 → 계좌별 평가액·비중·합계 표시. 매입단가가 있는 키움 계좌는 손익/수익률도 표시
- **CNN Fear & Greed 게이지** (여기에만 있음)
- 시세: Yahoo Finance v8 API + CORS 프록시(`corsproxy.io`) 폴백
- 새 계좌 추가 시 `ACCOUNTS` 배열에 한 줄 + 계좌 카드 한 개 추가
- (종목 매수/매도 신호·종목별 차트는 메인에서 빼고 → 각 계좌 페이지로 이동)

### `rebal_*.html` (토스·연금 리밸런싱)
- 좌측: 레짐/목표 배지 + 권장 구성 + 현재 보유 평가
- 우측: 보유 현황 입력 → 저장
- 권장 비교표 + 자산 변화 차트(Plotly) + 입력 기록 테이블
- ☁️ iCloud 폴더 연결(Chrome 전용, Mac 최초 1회)

### `kiwoom.html?acct=kr|us` (키움 수익률 추적)
- 보유 종목 P&L 표(수량·매입가·현재가·평가손익·수익률) + 계좌 요약(총매입/평가/손익)
- **매수/매도 신호 카드 + 종목별 차트**(주가&RSI / Drawdown) — 보유 종목 기준, 메인에서 이동해 온 기능
- 레버리지 판정은 `LEV_TICKERS` 집합으로 (TQQQ·QLD·SOXL 등 → 추세추종 신호 적용)
